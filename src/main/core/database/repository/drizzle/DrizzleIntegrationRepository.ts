...
export class DrizzleIntegrationRepository implements IntegrationRepository {
  ...
  async create(data: CreateIntegrationData): Promise<IntegrationRecord> {
    ...
    return selectById<IntegrationRecord>(this.db, integrations, integrations.integration_id, data.integrationId)
  }
  ...
  async update(
    ...
    await this.db.update(integrations).set(updateData).where(eq(integrations.integration_id, integrationId))
    return selectById<IntegrationRecord>(this.db, integrations, integrations.integration_id, integrationId)
  }
