...
Hydra는 Electron 기반의 경량 프로젝트/이슈 관리 데스크톱 앱입니다. 오프라인을 우선하며, 사용자가 자신의
...
- [`CLAUDE.md`](../CLAUDE.md) — 아키텍처와 컨벤션 요약이자 AI 어시스턴트 가이드
- `docs/design/` — DB 컨벤션, ERD, UI 설계
- `docs/project/` — 백로그, 버그, 마일스톤

## 핵심 개념 한 줄 요약

- **BYO-Database**: 앱이 DB를 내장하지 않습니다. 사용자가 PostgreSQL 또는 MySQL을 연결합니다.
- **2단계 인증**: 워크스페이스(공유 DB 연결)를 거쳐 앱 로그인(개인 계정)으로 이어집니다.
- **워크스페이스**: 하나의 DB 연결이 곧 하나의 워크스페이스이며, 여러 워크스페이스를 전환할 수 있습니다.
