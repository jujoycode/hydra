...
- 모든 작업은 최신 `main`에서 분기한 **짧은 브랜치**에서 진행하고, **PR로만** 병합합니다(CI green과 승인 1).
- 병합 후 소스 브랜치는 삭제합니다. 기본 병합 방식은 squash입니다.
- 릴리즈는 `main`에서 SemVer 태그(`vMAJOR.MINOR.PATCH`)로 만들며, 태그 push가 릴리즈 빌드를 트리거합니다.

멀티 DBMS 지원과 인증 모델 재설계 작업(`v3`, Phase 1~5, PR #27~#31)이 `main`으로 승격되었습니다(PR #32).

## Legacy 아카이브

과거 갈래들은 역사 보존을 위해 **`legacy/*`** 네임스페이스로 아카이브되어 있습니다. 삭제하지 않고
그대로 보존하므로 언제든 참조할 수 있습니다.
...
현재 원격에는 **`main`과 `legacy/{main,ui-v2,develop,docs}`** 만 남아 있습니다.

> `legacy/*`는 불변 아카이브입니다. 삭제하거나 그 위에서 새 작업을 시작하지 않습니다.
