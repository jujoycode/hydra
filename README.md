...
**Hydra**는 Electron으로 만든 가벼운 프로젝트·이슈 관리 데스크톱 앱입니다. 오프라인을 우선하고, 여러 워크스페이스를 지원하며, 오픈소스입니다. 데이터는 외부 클라우드가 아니라 사용자의 **PostgreSQL 또는 MySQL 8 데이터베이스**에 직접 연결해 보관합니다.
...
앱을 처음 실행하면 워크스페이스 연결 화면이 표시됩니다. DB 접속 정보를 입력해 연결하면, 빈 데이터베이스인 경우 관리자 셋업 화면으로 이동합니다. 첫 실행 흐름은 [Getting Started](./wiki/Getting-Started.md)에서 설명합니다.

## 🏗 아키텍처

Hydra는 Electron의 3-프로세스 모델을 따릅니다. 렌더러는 데이터베이스를 직접 다루지 않습니다. 모든 호출은 타입 안전 IPC 브리지를 거쳐 메인 프로세스로 전달되며, 데이터 접근은 메인이 전담합니다.
...
Hydra는 DB 연결을 곧 인증으로 보지 않습니다. 워크스페이스 연결과 앱 로그인을 분리합니다.

<p align="center">
  <img src="./docs/assets/readme/auth-2step.svg" alt="2단계 인증: 워크스페이스 연결 후 앱 로그인" width="95%" />
</p>

1. **워크스페이스 연결** — 공유 서비스 계정으로 DB에 접속합니다(host, port, dbName, dbms 및 자격증명). 접속 정보는 Electron safeStorage로 암호화해 저장합니다.
2. **앱 로그인** — 개인 계정으로 로그인합니다(`user_sn`, scrypt 해시 비밀번호, 세션).

빈 DB에 처음 연결하면 관리자 셋업 화면이 표시되고, 관리자가 앱 안에서 멤버를 생성합니다(DB ROLE은 사용하지 않습니다). 세션은 safeStorage로 영속화되며, "remember me"를 사용하면 만료가 연장됩니다. bootstrap 시 다시 검증합니다. 자세한 내용은 [Authentication](./wiki/Authentication.md)을 참고하세요.

## 🗄 데이터베이스 (PostgreSQL / MySQL 8)

워크스페이스를 추가할 때 DBMS를 선택합니다. 하나의 리포지토리 인터페이스를 DBMS별 어댑터가 구현하고, 연결 시 팩토리가 적절한 어댑터를 선택합니다.

<p align="center">
  <img src="./docs/assets/readme/multi-dbms.svg" alt="하나의 어댑터 인터페이스로 멀티 DBMS 지원" width="100%" />
</p>

런타임 서비스 계정에는 DML 권한만 있으면 됩니다(`ALL PRIVILEGES`는 부여하지 마세요). 스키마 마이그레이션은 연결 시 자동으로 실행되며, 스키마가 최신이면 마이그레이터를 건너뜁니다. 따라서 DML 전용 계정으로도 정상 동작합니다.
...
첫 연결 시, 그리고 앱 업그레이드 후 대기 중인 마이그레이션이 있을 때는 DDL 권한(`CREATE, ALTER, INDEX, REFERENCES`)을 가진 계정으로 한 번 연결해야 합니다. MySQL은 `utf8mb4` charset이 필수입니다.

> ⚠️ MySQL 워크스페이스에는 `drizzle-kit push`를 사용하지 마세요. 생성된 마이그레이션만 사용합니다(collation은 스키마 custom type이 소유합니다).
