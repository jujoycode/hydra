...
> 여기 Phase 1~6 중 다수는 이미 구현됐고(2026-06-13 코드 감사 기준), 멀티 DBMS/인증 재설계는 반영돼 있지 않다.
> "v1.1+ 고려 사항"과 리스크 섹션의 장기 비전은 여전히 참고 가치가 있다.

## Context

Hydra는 현재 약 40-50% 완성도의 Electron 기반 프로젝트/이슈 관리 데스크톱 앱이다. 기본 CRUD(프로젝트, 이슈, 멤버, 워크스페이스)와 대시보드가 구현되어 있지만, 협업 기능(코멘트, 알림), 고급 뷰(칸반), 외부 연동(GitHub), 테스트 커버리지가 부재한 상태. 5-20명 규모 팀을 타깃으로 조만간 오픈소스 공개를 목표로 한다.
...
**목표:** 모든 기존 페이지가 실제 데이터로 동작. UI 데드엔드 제거. 리치 텍스트 기반 마련.
...
**목표:** 이슈 분류 체계와 변경 이력 추적. 알림과 웹훅의 전제 조건 구축.
...
**목표:** 가장 임팩트 있는 시각적 기능(칸반)과 협업 백본(알림) 구현.
...
**목표:** 파워유저 기능 + 외부 연동 기반. 10명+ 팀에서 효율적 사용 가능하게.
...
**목표:** 공개 배포에 필요한 문서, 패키징, 잔여 페이지 완성.
