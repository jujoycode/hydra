...
> ⚠️ **개별 티켓의 완료 상태는 낡았다.** 2026-06-13 코드 감사 결과, Critical로 표기된 HYDRA-002(이슈 생성 API)·
> HYDRA-004(홈 실데이터) 등은 **이미 구현 완료**다. 현재 작업의 기준은
> **[`docs/project/milestones-v1.md`](./milestones-v1.md)** (v1.0.0 마일스톤)이며, 이 백로그는 개별 아이디어
> 참고용으로 남긴다. 실제 미완료 갭은 milestones-v1.md의 "현황" 섹션을 신뢰할 것.
